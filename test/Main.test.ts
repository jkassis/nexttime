...
import { Recurrence, TimeUnit, DateTimePartK, RuleConfDayOfWeek, RuleConfWeekOfMonth, DateTimePartVDayOfWeek } from '../code/Main.js'
...
it('test recurrence for sundays', () => {
    ...
    var sundays = new Recurrence({
        ...
        rules: [{ timePart: DateTimePartK.DayOfWeek, whiteList: [DateTimePartVDayOfWeek.Su] } as RuleConfDayOfWeek],
        utcOffset: 7
    })
    ...
    var sundays = new Recurrence({
        frequency: TimeUnit.Day,
        rules: [
            { timePart: DateTimePartK.DayOfWeek, whiteList: [DateTimePartVDayOfWeek.Su] } as RuleConfDayOfWeek,
            { timePart: DateTimePartK.WeekOfMonth, whiteList: [0, 2] } as RuleConfWeekOfMonth
        ],
        utcOffset: 7
    })
    var occurrences = sundays.occurrences(bidenDay.toMillis(), null)
